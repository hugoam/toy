...
namespace mud
{
    ...
    // TileWorld
    {
        ...
        static Class cls = { type<TileWorld>(),
            ...
            // constructors
            {
                { type<TileWorld>(), [](Ref ref, array<Var> args) { new(&val<TileWorld>(ref)) TileWorld( val<std::string>(args[0]), val<mud::JobSystem>(args[1]) ); }, { { "name", var(std::string()) }, { "job_system", Ref(type<mud::JobSystem>()) } } }
            },
            // copy constructor
            {
            },
