...
namespace mud
{
    ...
    template <> _PLATFORM_EXPORT Type& type<ComponentHandle<Bullet>>() { static Type ty("ComponentHandle<Bullet>", type<mud::Entity>(), sizeof(ComponentHandle<Bullet>)); return ty; }
    template <> _PLATFORM_EXPORT Type& type<ComponentHandle<Crate>>() { static Type ty("ComponentHandle<Crate>", type<mud::Entity>(), sizeof(ComponentHandle<Crate>)); return ty; }
    template <> _PLATFORM_EXPORT Type& type<ComponentHandle<Human>>() { static Type ty("ComponentHandle<Human>", type<mud::Entity>(), sizeof(ComponentHandle<Human>)); return ty; }
    template <> _PLATFORM_EXPORT Type& type<ComponentHandle<Lamp>>() { static Type ty("ComponentHandle<Lamp>", type<mud::Entity>(), sizeof(ComponentHandle<Lamp>)); return ty; }
