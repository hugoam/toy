...
namespace mud
{
    ...
    template <> _GODOT_EXPORT Type& type<mud::ComponentHandle<Bullet>>() { static Type ty("ComponentHandle<Bullet>", type<mud::Entity>(), sizeof(mud::ComponentHandle<Bullet>)); return ty; }
    template <> _GODOT_EXPORT Type& type<mud::ComponentHandle<Crate>>() { static Type ty("ComponentHandle<Crate>", type<mud::Entity>(), sizeof(mud::ComponentHandle<Crate>)); return ty; }
    template <> _GODOT_EXPORT Type& type<mud::ComponentHandle<Human>>() { static Type ty("ComponentHandle<Human>", type<mud::Entity>(), sizeof(mud::ComponentHandle<Human>)); return ty; }
    template <> _GODOT_EXPORT Type& type<mud::ComponentHandle<Lamp>>() { static Type ty("ComponentHandle<Lamp>", type<mud::Entity>(), sizeof(mud::ComponentHandle<Lamp>)); return ty; }
}
